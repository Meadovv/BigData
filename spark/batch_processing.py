...
# MAIN SPARK JOB
hdfs_ip = "172.25.0.2"
spark_master_ip = "172.25.0.31"
spark = SparkSession.builder.master(f"spark://{spark_master_ip}:7077").getOrCreate()
spark.conf.set("spark.hadoop.fs.defaultFS", f"hdfs://{hdfs_ip}:9000")
spark.conf.set("spark.hadoop.fs.default.name", f"hdfs://{hdfs_ip}:9000")
spark.conf.set("spark.hadoop.fs.hdfs.impl", "org.apache.hadoop.hdfs.DistributedFileSystem")
spark.conf.set("spark.hadoop.fs.hdfs.server", f"hdfs://{hdfs_ip}:9000")
spark.conf.set("spark.hadoop.ipc.client.connect.max.retries", "10")
# Load files from HDFS
hdfs_file_path = [
    f"hdfs://{hdfs_ip}:9000/meadow/houses/bds.jsonl",
    f'hdfs://{hdfs_ip}:9000/meadow/houses/guland.jsonl',
    f'hdfs://{hdfs_ip}:9000/meadow/houses/nhadatviet.jsonl',
    f'hdfs://{hdfs_ip}:9000/meadow/houses/ibds.jsonl',
]
# spark.sparkContext.setLogLevel("DEBUG")
df = spark.read \
    .option("spark.hadoop.fs.defaultFS", f"hdfs://{hdfs_ip}:9000") \
    .json(hdfs_file_path)
print("Done loading.")
...
# df.drop("id")
# # Save to HDFS
# output_path = 'hdfs://20.239.82.205:9000/haihp02/real_estate_data/processed_test.jsonl'
