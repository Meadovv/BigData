...
spark = SparkSession.builder \
    .master("spark://172.25.0.31:7077") \
    .config("es.index.auto.create", "true") \
    .getOrCreate()
# Set up data stream
topic = "bds"
kafka_config = {
    "kafka.bootstrap.servers": "172.24.181.57:9192,172.24.181.57:9292,172.24.181.57:9392",
    "subscribe": topic,
...
checkpoints = f"hdfs://172.25.0.2:9000/meadow/spark/elasticsearch_log_checkpoint/{topic}"

# Output two queries, one to console and one to elasticsearch
console_query = final_df.writeStream \
    .outputMode("append") \
    .format("console") \
    .option("checkpointLocation", f"{checkpoints}/console") \
    .start()

es_config = {
    "es.nodes": "172.25.0.41,172.25.0.42,172.25.0.43",
    "es.port": "9200",
    "es.resource": f"houses_in_hanoi_from_{topic}_with_stream_processing",
