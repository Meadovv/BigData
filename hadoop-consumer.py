...
logging.info(f'NAMENODE_URL: {NAMENODE_URL}')
logging.info(f'HDFS_USER: {HDFS_USER}')
logging.info(f'BOOSTRAP_SERVERS: {BOOSTRAP_SERVERS}')
logging.info(f'CONSUMER_GROUP_ID: {CONSUMER_GROUP_ID}')
logging.info(f'SUBSCRIBE_LIST: {SUBSCRIBE_LIST}')
logging.info(f'POLL_TIMEOUT: {POLL_TIMEOUT}')

try:
    hdfs_client = InsecureClient(url=NAMENODE_URL)
...
def push_to_hdfs(hdfs_file_path, msg):
    if not check_hdfs_path_existence(hdfs_file_path):
        ...
        hdfs_client.makedirs(os.path.dirname(hdfs_file_path))
    else:
        append = True
...
if __name__ == '__main__':
    ...
    while True:
        ...
        hdfs_file_path = f'/{HDFS_USER}/houses/{msg.topic()}.jsonl'
